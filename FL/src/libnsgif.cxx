...
static unsigned char/*bool*/ gif_next_LZW(gif_animation *gif);
...
static unsigned char/*bool*/ zero_data_block = NSGIF_FALSE;
static unsigned char/*bool*/ get_done;

/*	Whether to clear the decoded image rather than plot
*/
static unsigned char/*bool*/ clear_image = NSGIF_FALSE;
...
gif_result gif_initialise(gif_animation *gif, int size, unsigned char *data)
{
	...
	*/
	if (gif->buffer_position == 0) {
		...
		*/
		gif->global_colour_table = (unsigned int *)calloc(GIF_MAX_COLOURS, sizeof(unsigned int));
		gif->local_colour_table = (unsigned int *)calloc(GIF_MAX_COLOURS, sizeof(unsigned int));
...
static gif_result gif_initialise_frame(gif_animation *gif)
{
	...
	unsigned char/*bool*/ first_image = NSGIF_TRUE;
	gif_result return_value;
	...
	*/
	...
	gif->frames[frame].display = NSGIF_FALSE;
	gif->frames[frame].virgin = NSGIF_TRUE;
	gif->frames[frame].disposal_method = 0;
	gif->frames[frame].transparency = NSGIF_FALSE;
	gif->frames[frame].frame_delay = 100;
	gif->frames[frame].redraw_required = NSGIF_FALSE;
	...
	*/
	gif->buffer_position = gif_data - gif->gif_data;
	gif->frame_count = frame + 1;
	gif->frames[frame].display = NSGIF_TRUE;

	/*	Check if we've finished
	*/
	if (gif_bytes < 1)
		return GIF_INSUFFICIENT_FRAME_DATA;
	else if (gif_data[0] == GIF_TRAILER) return GIF_OK;
	return GIF_WORKING;
}
		...
		*/
		...
		case GIF_EXTENSION_GRAPHIC_CONTROL:
			...
			if (gif_data[2] & GIF_TRANSPARENCY_MASK) {
				gif->frames[frame].transparency = NSGIF_TRUE;
...
gif_result gif_decode_frame(gif_animation *gif, unsigned int frame)
{
	...
	*/
	if (gif->frames[frame].display == NSGIF_FALSE) {
	...
	*/
	if (gif->frames[frame].virgin) {
		if (gif->bitmap_callbacks.bitmap_test_opaque)
			...
			gif->frames[frame].opaque = NSGIF_FALSE;
		gif->frames[frame].virgin = NSGIF_FALSE;
	}
...
void gif_finalise(gif_animation *gif)
{
	...
	*/
	...
	gif->frame_image = NULL;
	if ( gif->frames != NULL ) free(gif->frames);
	gif->frames = NULL;
	if ( gif->local_colour_table != NULL ) free(gif->local_colour_table);
	gif->local_colour_table = NULL;
	if ( gif->global_colour_table != NULL ) free(gif->global_colour_table);
	gif->global_colour_table = NULL;
}
