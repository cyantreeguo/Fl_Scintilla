...
Fl_Window *Fl_Widget::top_window() const
{
	...
	while (w->parent()) {
		w = w->parent();        // walk up the widget hierarchy to top-level item
	}
...
#if __FLTK_IPHONEOS__
void Fl_Window::setOrientationsEnabled (int newOrientations)
{
	// Dodgy set of flags being passed here! Make sure you specify at least one permitted orientation.
	if (newOrientations != 0 && (newOrientations & ~allOrientations) == 0) allowedOrientations = newOrientations;
}

bool Fl_Window::isOrientationEnabled (DisplayOrientation orientation) const
{
	// Make sure you only pass one valid flag in here...
	if (orientation == upright || orientation == upsideDown || orientation == rotatedClockwise || orientation == rotatedAntiClockwise) return (allowedOrientations & orientation) != 0;
	return false;
}
