//
// "$Id: Fl_win32.cxx 10159 2014-05-23 16:47:21Z manolo $"
//
...
{
	if (!s_wsock_mod) {
		s_wsock_mod = LoadLibraryA(WSCK_DLL_NAME);
		if (s_wsock_mod==NULL)
			Fl::fatal("FLTK Lib Error: %s file not found! Please check your winsock dll accessibility.\n",WSCK_DLL_NAME);
		s_wsock_select = (fl_wsk_select_f) GetProcAddressA(s_wsock_mod, "select");
		fl_wsk_fd_is_set = (fl_wsk_fd_is_set_f) GetProcAddressA(s_wsock_mod, "__WSAFDIsSet");
	}
	return s_wsock_mod;
}
*/
...
static HMODULE get_imm_module()
{
	if (!s_imm_module) {
		...
		flImmGetContext = (flTypeImmGetContext)GetProcAddressA(s_imm_module, "ImmGetContext");
		flImmSetCompositionWindow = (flTypeImmSetCompositionWindow)GetProcAddressA(s_imm_module, "ImmSetCompositionWindow");
		flImmReleaseContext = (flTypeImmReleaseContext)GetProcAddressA(s_imm_module, "ImmReleaseContext");
		flImmIsIME = (flTypeImmIsIME)GetProcAddressA(s_imm_module, "ImmIsIME");
	}
	return s_imm_module;
}
*/
...
void fl_set_spot(int font, int size, int X, int Y, int W, int H, Fl_Window *win)
{
	...
	//get_imm_module();
	HIMC himc = ImmGetContext(fl_xid(tw));
	...
	if (himc) {
		...
		ImmReleaseContext(fl_xid(tw), himc);
	}
}
	...
	if (nfds) {
		...
		if ( select(maxfd+1,&fdt[0],&fdt[1],&fdt[2],&t)) {
			...
			for (int i = 0; i < nfds; i ++) {
				SOCKET f = fd[i].fd;
				short revents = 0;
				if (__WSAFDIsSet(f, &fdt[0])) revents |= FL_READ;
				if (__WSAFDIsSet(f, &fdt[1])) revents |= FL_WRITE;
				if (__WSAFDIsSet(f, &fdt[2])) revents |= FL_EXCEPT;
...
{
	char buf[8];
	buf[GetLocaleInfoA(id, LOCALE_IDEFAULTANSICODEPAGE, buf, 8)] = 0;
	return atol(buf);
}
*/
		...
		*o = 0;
		...
		if (strcmp(type, Fl::clipboard_plain_text) == 0) { // we want plain text from clipboard
			...
			/*else if ((h = GetClipboardData(CF_ENHMETAFILE))) { // if there's an enhanced metafile in clipboard
				...
				fl_begin_offscreen(off);
				fl_color(FL_WHITE);
				fl_rectf(0,0,width, height); // draw white background
	...
	if (fallback) {
		if (w->border() && !w->parent()) {
			if (w->size_range_set && (w->maxw != w->minw || w->maxh != w->minh)) {
				...
				bx = 0;//GetSystemMetrics(SM_CXFIXEDFRAME);
				by = 0;//GetSystemMetrics(SM_CYFIXEDFRAME);
			} else {
				ret = 1;
				/*
...
void Fl_X::make_fullscreen(int X, int Y, int W, int H)
{
	...
	SetWindowPos(xid, HWND_TOP, X, Y, W, H, SWP_FRAMECHANGED);
}
...
Fl_X* Fl_X::make(Fl_Window* w)
{
	...
	if (!class_name_list.has_name(class_name)) {
		...
		//if (!w->icon())w->icon((void *)LoadIcon(NULL, IDI_APPLICATION));
		wcw.hIcon = (HICON)w->icon();
...
void Fl::repeat_timeout(double time, Fl_Timeout_Handler cb, void* data)
{
	...
	if ( !s_TimerWnd ) {
		...
		s_TimerWnd = CreateWindowEx(WS_EX_TOOLWINDOW,
		                            timer_class, _T(""),
		                            WS_POPUP,
		                            0, 0, 0, 0,
...
void Fl_X::set_default_icons(HICON big_icon, HICON small_icon)
{
	...
	if (big_icon != NULL)
		default_big_icon = NULL;//CopyIcon(big_icon);
	if (small_icon != NULL)
		default_small_icon = NULL;//CopyIcon(small_icon);
}
...
void Fl_Window::icons(HICON big_icon, HICON small_icon)
{
	...
	if (big_icon != NULL)
		icon_->big_icon = NULL;//CopyIcon(big_icon);
	if (small_icon != NULL)
		icon_->small_icon = NULL;//CopyIcon(small_icon);
