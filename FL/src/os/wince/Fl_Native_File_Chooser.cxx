...
//  Unix2Win(_ofn.lpstrFile);
	...
	if ( oldcwdsz > 0 ) {
		...
		if (GetCurrentDirectoryA(oldcwdsz, oldcwd) == 0 ) {
			free(oldcwd);
			oldcwd = 0;
		}
	}
...
int Fl_Native_File_Chooser::showdir()
{
	...
	_binf.lpfn = Dir_CB;
	// OPEN BROWSER
	LPITEMIDLIST pidl = NULL;// SHBrowseForFolder(&_binf); // FIXIT:
/*
修改方法如下:
http://hi.baidu.com/yuzhi2217/item/7b6cc7e40e5f2b3a4cdcaf2f
...
ofn.lpstrInitialDir = _T(" Files\\MyApp");
...
ofn.lpstrInitialDir = _T(" Files"); // 这里可以指定打开的默认目录
ofn.lpstrTitle = _T("Select a bitmap");
...
static int count_filters(const char *filter)
{
	...
	while (*in) {
		...
		case '\\':			// escape next character
			++in;
			if ( *in == 0 ) continue;	// skip escape. EOL? done
...
void Fl_Native_File_Chooser::parse_filter(const char *in)
{
	...
	char *wildprefix = new char[slen*2];
	wildprefix[0] = 0;
	char *comp       = new char[slen*2];
	comp[0] = 0;
	char *name       = new char[slen*2];
	name[0] = 0;
