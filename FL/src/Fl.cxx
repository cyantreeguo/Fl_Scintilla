//
// "$Id: Fl.cxx 10159 2014-05-23 16:47:21Z manolo $"
//
...
// hence we must include winsock2.h before Fl.H (A.S. Dec. 2010, IMM May 2011)
...
extern double fl_ios_flush_and_wait(double time_to_wait);
#endif
	...
	if (window()) {
		...
		if (align() && !(align() & FL_ALIGN_INSIDE) && window()->shown()) {
			...
			case FL_ALIGN_TOP_LEFT:
				...
				break;
			case FL_ALIGN_TOP:
				window()->damage(FL_DAMAGE_EXPOSE, x()+(w()-W)/2, y()-H, W, H);
				break;
			case FL_ALIGN_TOP_RIGHT:
				window()->damage(FL_DAMAGE_EXPOSE, x()+w()-W, y()-H, W, H);
				break;
			...
			case FL_ALIGN_RIGHT_TOP:
				...
				break;
			case FL_ALIGN_LEFT:
				window()->damage(FL_DAMAGE_EXPOSE, x()-W, y()+(h()-H)/2, W, H);
				break;
			case FL_ALIGN_RIGHT:
				window()->damage(FL_DAMAGE_EXPOSE, x()+w(), y()+(h()-H)/2, W, H);
				break;
			case FL_ALIGN_LEFT_BOTTOM:
				window()->damage(FL_DAMAGE_EXPOSE, x()-W, y()+h()-H, W, H);
				break;
