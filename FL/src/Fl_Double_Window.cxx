...
char fl_can_do_alpha_blending()
{
	...
	HMODULE hMod = LoadLibraryA("MSIMG32.DLL");
...
*/
/*
typedef BOOL (WINAPI* fl_alpha_blend_func)(HDC,int,int,int,int,HDC,int,int,int,int,FL_BLENDFUNCTION);
static fl_alpha_blend_func fl_alpha_blend = NULL;
*/
static BLENDFUNCTION blendfunc = { 0, 0, 255, 1};
...
char fl_can_do_alpha_blending()
{
	...
	*/
	...
	/*COLORREF set = */
	SetPixel(new_gc, 0, 0, 0x01010101);
	BOOL alpha_ok = AlphaBlend(dc, 0, 0, 1, 1, new_gc, 0, 0, 1, 1, blendfunc);
	RestoreDC(new_gc, save);
	DeleteDC(new_gc);
	DeleteObject(bm);
	ReleaseDC(0L, dc);
