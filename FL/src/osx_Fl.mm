//
// "$Id: Fl_cocoa.mm 10159 2014-05-23 16:47:21Z manolo $"
//
...
static NSInteger max_normal_window_level(void)
{
	...
	for (x = Fl_X::first; x; x = x->next) {
		...
		if (win->modal() || win->non_modal()) continue;
		level = [cw level];
...
static NSInteger modal_window_level(void)
{
	...
	if (level < NSModalPanelWindowLevel) return NSModalPanelWindowLevel;

	// Need some room for non-modal windows
	level += 2;

	// We cannot exceed this
	if (level > CGShieldingWindowLevel()) return CGShieldingWindowLevel();
...
static NSInteger non_modal_window_level(void)
{
	...
	level = max_normal_window_level();
	if (level < NSFloatingWindowLevel) return NSFloatingWindowLevel;
...
static void cocoaMouseHandler(NSEvent *theEvent)
{
	...
	switch (etype) {
		...
		case NSOtherMouseDown:
			...
			if ([theEvent clickCount] > 1) Fl::e_clicks++;
			else Fl::e_clicks = 0;
			// fall through
		case NSLeftMouseUp:
		case NSRightMouseUp:
...
- (void)applicationDidUpdate: (NSNotification *)aNotification
{
	if (im_enabled != -1) {
		...
		if (newDoc != currentDoc) {
			...
			if (im_enabled) TSMRemoveDocumentProperty(doc, kTSMDocumentEnabledInputSourcesPropertyTag);
			else {
...
+ (void)sendEvent: (NSEvent *)theEvent
{
	if (fl_send_system_handlers(theEvent)) return;
...
void Fl::enable_im()
{
	...
	} else KeyScript(smKeyEnableKybds);
}
...
void Fl::disable_im()
{
	...
	if (fl_mac_os_version >= 100500) {
		((FLAppDelegate *)[NSApp delegate])->currentDoc = NULL;
		[NSApp updateWindows]; // triggers [FLAppDelegate applicationDidUpdate]
	} else KeyScript(smEnableRomanKybdsOnly);
}
...
int Fl_X::set_cursor(Fl_Cursor c)
{
	...
	switch (c) {
		...
		case FL_CURSOR_N:
			cursor = [NSCursor resizeUpCursor]; break;
		case FL_CURSOR_E:
			cursor = [NSCursor resizeRightCursor]; break;
		case FL_CURSOR_W:
			cursor = [NSCursor resizeLeftCursor]; break;
		case FL_CURSOR_S:
			cursor = [NSCursor resizeDownCursor]; break;
		default:
			return 0;
	}
...
int Fl_X::set_cursor(const Fl_RGB_Image *image, int hotx, int hoty)
{
	...
	if ((hotx < 0) || (hotx >= image->w())) return 0;
...
// a compilation warning appears with SDK 10.5, so we require SDK 10.6 instead
	...
	if (to_quartz) img = Fl_X::CGImage_from_window_rect(win, 0, -bt, win->w(), bt);
	else bitmap = Fl_X::bitmap_from_window_rect(win, 0, -bt, win->w(), bt, &bpp);
